"""One-shot fusion strategies for heterogeneous CNN federated learning."""
...
def _set_averaged_weights(server, accumulators):
    (
        conv_layers, bn_layers, dense_layers,
        conv_sum, conv_count, bn_sum, bn_count,
        dense_w_sum, dense_w_count, dense_b_sum, dense_b_count,
    ...
    for idx, layer in enumerate(conv_layers):
        layer.set_weights([safe_div(conv_sum[idx], conv_count[idx])])
...
def fuse_stitchfl(
    ...
    for idx, layer in enumerate(server_bn):
        gamma = safe_div(bn_sum[idx][0], bn_count[idx][0])
        beta = safe_div(bn_sum[idx][1], bn_count[idx][1])
...
def fuse_halofl(
    ...
    _set_averaged_weights(server, accumulators)
    if verbose:
        print("    UIL-weighted aggregation (activation ratio + magnitude)")
    return server
...
def fuse_subflot(
    ...
    _set_averaged_weights(server, accumulators)
    if verbose:
        print(f"    OT channel alignment (Sinkhorn, reg={reg}) before averaging")
    return server
